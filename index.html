...
// Sempre chame isso quando alterar o estado do botão de sair
atualizarTutorial();

// Verifica se já foi visto
const jaViu = localStorage.getItem("tutorial_visto");

// Exibe somente se necessário
...
function animateRotation() {
    ...
    const smooth = 0.2; // Suavidade (0.05 ~ mais suave | 0.2 ~ mais rápido)
...
escalaWrapper.addEventListener('pointerup', (e) => {
    ...
    // --- FLIP APENAS COM BOTÃO ESQUERDO ---
    if (e.button === 0) {
        if (!dragging) {
            isFlipped = !isFlipped;
            resetTransform();
    ...
    // --- FINALIZAR ARRASTE DO BOTÃO DIREITO ---
    if (e.button === 2) {
        draggingRight = false;
...
tutorial.style.display = 'none';  // ← esconde o tutorial
